...
@FileName: option.py
@Software: PyCharm
Load training options from JSON config file
"""
...
class Options:
    def __init__(self, config_path="config.json", isTrain=True):
    ...
    def _load_json(self):
        ...
        # merge base + train
        merged = {**cfg.get("base", {}), **cfg.get("train", {})}
        self.opt = SimpleNamespace(**merged)
        self.opt.isTrain = self.isTrain

    def _post_process(self):
        # ===== device =====
        self.opt.device = torch.device(
            "cuda" if torch.cuda.is_available() and self.opt.device == "gpu" else "cpu"
        )

        # ===== gpu ids =====
        gpu_ids = []
        for i in str(self.opt.gpu_ids).split(","):
            i = int(i)
            if i >= 0:
                gpu_ids.append(i)
        ...
        if self.opt.name == "experiment_name":
            self.opt.name = f"{self.opt.model}/{self.opt.dataset}"

        # ===== output dirs =====
        train_dir = os.path.join(self.opt.outf, self.opt.name, "train")
        test_dir = os.path.join(self.opt.outf, self.opt.name, "test")
        os.makedirs(train_dir, exist_ok=True)
        os.makedirs(test_dir, exist_ok=True)

        # ===== save opt.txt =====
        opt_file = os.path.join(train_dir, "opt.txt")
...
#         self.parser.add_argument('--niter', type=int, default=5, help='number of epochs to train for') # Số lần training default 50
