...
def load_data(opt):
    ...
    if opt.dataset in ['unsw']:
        ...
        # Convert to float32 to avoid object type issues
        data = np.array(data, dtype=np.float32)
        x_test = np.array(x_test, dtype=np.float32)
        y_test = np.array(y_test, dtype=np.float32)
        
        y_train = np.zeros(len(data), dtype=np.float32)
        ...
        # FIX: Convert to float32 to avoid object type issues
        data = np.array(data, dtype=np.float32)
        x_test = np.array(x_test, dtype=np.float32)
        y_test = np.array(y_test, dtype=np.float32)
        ...
        y_train = np.zeros(len(train_normal), dtype=np.float32)
        y_train_tensor = torch.FloatTensor(y_train)
        y_test_tensor = torch.FloatTensor(y_test)
