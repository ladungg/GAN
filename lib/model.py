...
class BaseModel():
    ...
    def __init__(self, opt, dataloader):
        ...
        if not hasattr(opt, 'display_id'):
            opt.display_id = 0
        if not hasattr(opt, 'display'):
            opt.display = False
        if not hasattr(opt, 'print_freq'):
            opt.print_freq = 100
        if not hasattr(opt, 'save_image_freq'):
    ...
    def test_malware(self,testload):
        ...
        with torch.no_grad():
            ...
            lamb = getattr(self.opt, 'lamb', 0.5)  # Default: 0.5 means equal weight
            prob = lamb * scores_hidden + (1 - lamb) * scores_adv
            # Measure inference time.
            self.times = np.array(self.times)
            self.times = np.mean(self.times[:100] * 1000)
            # auc, eer = roc(self.gt_labels, self.an_scores)
...
class FlowGANAnomaly(BaseModel):
    ...
    def __init__(self, opt, dataloader):
        ...
        self.flow_shape,self.labels = next(self.dataloader_shape)
        ...
        if torch.cuda.is_available():
            if isinstance(self.opt.device, str) and self.opt.device.startswith('cuda:'):
                device_id = int(self.opt.device.split(':')[1])
            elif isinstance(self.opt.device, int):
                device_id = self.opt.device
            else:
                device_id = None
            self.netg = NetG(self.opt, self.feature_number).cuda(device_id)
            self.netd = NetD(self.opt, self.feature_number).cuda(device_id)
        else:
            print("CUDA not available, using CPU")
            self.netg = NetG(self.opt, self.feature_number).cpu()
            self.netd = NetD(self.opt, self.feature_number).cpu()
